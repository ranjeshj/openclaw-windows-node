...
For each design screen the designer sent us, this tool:
  ...
  2. Resizes both the designer reference PNG and the rendered capture
     to a common size (1568 x 2106 — the designer canvas) so the diff
     is apples-to-apples even though the preview window renders at a
     smaller dip size and is then upscaled by per-monitor DPI.
  3. Writes three artifacts under ``out/v2-visual/<page>/``:

       * ``expected.png``  the designer reference
       * ``actual.png``    the rendered capture
       * ``diff.png``      a 3-up: expected | actual | per-pixel overlay
       * ``report.json``   SSIM, mean dE, % pixels above tolerance,
                           and a list of the largest mismatched bounding
                           boxes (handy when the eye misses a 1px
                           misalignment that compounds across a list)

The agent (and humans) are expected to ``view`` ``diff.png`` after every
implementation change, enumerate visible discrepancies, fix them, and
re-run until SSIM >= 0.95 on the content rect AND no mismatched bbox
larger than 24 x 24 px lives outside the documented permitted regions.
...
The script is intentionally side-effect free outside the build tree:
it never modifies designer references and never writes to ``src/``.
"""
